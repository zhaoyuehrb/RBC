...
totalMappedReads = 0
rpkmfile = rootdirectory+'../geneTXCD_RPKMoutput_processedData_'+rootdirectory[:-1].split('/')[-1]+'.txt'
...
for chrom in chrList:
    ...
    for gene in chromToGenes[chrom]:
        ...
        #if accNum in commonGeneToRawReads and ORFlength >= 600 and accNum not in histoneAccNums:
...
base_outfileString = 'AUGbaseToReads600_'+date+'_'+transfection+'_'+'mir'+'_rna'+str(rnaseq_threshold)+'fp'+str(footprint_threshold)+'.txt'
base_outfile = open(base_outfileString, 'w')
base_linesWritten = 0
codon_outfileString = 'AUGcodonToReads600_'+date+'_'+transfection+'_'+'mir'+'_rna'+str(rnaseq_threshold)+'fp'+str(footprint_threshold)+'.txt'
