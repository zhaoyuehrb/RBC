...
def getDetails(infileString):
    ...
    for line in infileLines[:-1]:
        ...
        if chrom == 'chrY':
            chrYreads += readNum
            chrYhits += 1
        if rRNAlabel == 'NA' and otherlabel == 'NA' and chrom != 'chrM' and chrom != 'chrY' and chrom != 'chrE':
            ## this hit could potentially be taken for a gene
            potentialGeneReads += readNum
            potentialGeneHits += 1
        
    return totalReads, rRNAreads, otherRNAreads, chrMreads, chrYreads, chrEreads, potentialGeneReads, mt_rRNA_reads, mt_tRNA_reads, mt_ORF_reads, totalHits, rRNAhits, otherRNAhits, chrMhits, chrYhits, chrEhits, potentialGeneHits, mt_rRNA_hits, mt_tRNA_hits, mt_ORF_hits
#########################################################################################################################


rootdirectory = sys.argv[1]
genomeDirectoryCore = (rootdirectory[:-1].split('/')[-1])[0:12]
genomeDirectory = rootdirectory
corefilename_genome = genomeDirectoryCore + "_genome"
...
for avaiLength in avaiLengths:
    ...
    totalReads, rRNAreads, otherRNAreads, chrMreads, chrYreads, chrEreads, potentialGeneReads, mt_rRNA_reads, mt_tRNA_reads, mt_ORF_reads, totalHits, rRNAhits, otherRNAhits, chrMhits, chrYhits, chrEhits, potentialGeneHits, mt_rRNA_hits, mt_tRNA_hits, mt_ORF_hits = getDetails(filePath)
    print 'Reads: ', totalReads, rRNAreads, otherRNAreads, chrMreads, chrYreads, chrEreads, potentialGeneReads, mt_rRNA_reads, mt_tRNA_reads, mt_ORF_reads
    print 'Hits: ', totalHits, rRNAhits, otherRNAhits, chrMhits, chrYhits, chrEhits, potentialGeneHits, mt_rRNA_hits, mt_tRNA_hits, mt_ORF_hits
    combineReads += totalReads
    combineHits += totalHits
    combineYreads += chrYreads
    combineMreads += chrMreads
    combineEreads += chrEreads
...
print 'Combined chrE reads: ', combineEreads
