...
plot_volcano <- function(res_tableDE, type, day,list_of_genes,label,folder_label,xlims,ylims){
  ...
  right <- unname(quantile(res_tableDE$log2FoldChange,0.9995))
  left <- unname(quantile(res_tableDE$log2FoldChange,0.0005))
  ...
  ggplot(res_tableDE,aes(x = log2FoldChange, y = -log10(padj))) +
    ...
    geom_text_repel(#data = subset(res_tableDE, ((log2FoldChange > 1 | log2FoldChange < -1) & (FALSE))),
      data = subset(res_tableDE, SYMBOL %in% c('AHSP','VEGFA','ACTB','ALAS2','SLC4A1','TMEM175') | (-log10(padj)>up) | (log2FoldChange>right) ), #for rna autophagy
      #data = subset(res_tableDE,  ((log2FoldChange>right) | (log2FoldChange<left) | (-log10(padj)>up))),
  ...
  #save_dir <- paste('/Users/Yue/Documents/HGLab/deseq_yue/Proper Version/Plots_redo/',label,'/',sep = '')
  ...
  filepath <- paste(type,' ',label,'.png',sep = '')
...
plot_volcano(res_0_S15,'ER S15 T0',1,c(),'','',xlims=c(-5,5), ylims=c(0,20))
...
plot_volcano(res_2_against_0_RNA,'RNA T2 against T0',1,c(),'','',xlims=c(-11,11), ylims=c(0,300))
