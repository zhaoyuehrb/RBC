...
plot_volcano <- function(res_tableDE, type, day,list_of_genes,label,folder_label,xlims,ylims){
  ...
  right <- unname(quantile(sub_dat$log2FoldChange,0.9))
  left <- unname(quantile(sub_dat$log2FoldChange,0.1))
  up <- unname(quantile(-log10(sub_dat$padj),0.8))
  ...
  ggplot(res_tableDE,aes(x = log2FoldChange, y = -log10(padj))) +
    ...
    geom_text_repel(#data = subset(res_tableDE, ((log2FoldChange > 1 | log2FoldChange < -1) & (FALSE))),
                    ...
                    data = subset(res_tableDE, (SYMBOL %in% list_of_genes) & ((log2FoldChange>right) | (log2FoldChange<left) | (-log10(padj)>up))),
  ...
  dir.create(file.path(getwd(), save_dir), showWarnings = FALSE)
  ...
  print(length(which(res_tableDE$log2FoldChange> 1 & res_tableDE$SYMBOL %in% list_of_genes)))
  print(length(which(res_tableDE$log2FoldChange< -1 & res_tableDE$SYMBOL %in% list_of_genes)))
  print(length(which(res_tableDE$log2FoldChange>= -1 & res_tableDE$log2FoldChange <=1 & res_tableDE$SYMBOL %in% list_of_genes)))
}
...
plot_volcano(res_1_L24,'ER L24 T1',1,c(),'','',xlims=c(-5,5), ylims=c(0,20))
plot_volcano(res_2_L24,'ER L24 T2',1,c(),'','',xlims=c(-5,5), ylims=c(0,10))

plot_volcano(res_0_S15,'ER S15 T0',1,c(),'','',xlims=c(-5,5), ylims=c(0,11))
plot_volcano(res_1_S15,'ER S15 T1',1,c(),'','',xlims=c(-5,5), ylims=c(0,20))
plot_volcano(res_2_S15,'ER S15 T2',1,c(),'','',xlims=c(-5,5), ylims=c(0,10))
...
plot_volcano(L24_0_S15,'L24 against S15 T0',1,c(),'','',xlims=c(-5,5),ylims=c(0,25))
plot_volcano(L24_1_S15,'L24 against S15 T1',1,c(),'','',xlims=c(-5,5),ylims=c(0,25))
plot_volcano(L24_2_S15,'L24 against S15 T2',1,c(),'','',xlims=c(-5,5),ylims=c(0,25))


plot_volcano(res_1_against_0_RNA,'RNA T1 against T0',1,c(),'','',xlims=c(-6,6), ylims=c(0,50))
plot_volcano(res_2_against_0_RNA,'RNA T2 against T0',1,c(),'','',xlims=c(-11,11), ylims=c(0,120))


plot_volcano(res_1_against_0_RPF,'RPF T1 against T0',1,c(),'','',xlims=c(-7,7), ylims=c(0,100))
plot_volcano(res_2_against_0_RPF,'RPF T2 against T0',1,c(),'','',xlims=c(-11,11), ylims=c(0,200))


plot_volcano(res_1_te_0,'TE T1 against T0',1,c(),'','',xlims=c(-5,5), ylims=c(0,10))
plot_volcano(res_2_te_0,'TE T2 against T0',1,c(),'','',xlims=c(-5,5), ylims=c(0,10))
