...
acc2eff = read.csv('../../../MOTIF/SeqLogo/gene_likelihood/ER_S15_t0_enriched_likelihood.csv')
rownames(acc2eff) <- acc2eff[,4]
acc2eff[,1]<-NULL
acc2eff[,2]<-NULL
acc2eff[,2]<-NULL
effs <- c()
...
plot_volcano <- function(res_tableDE, type, day,list_of_genes,label,folder_label,xlims,ylims){
  ...
  label <- 'seqLogo_S15_t0_enrich'
  res_tableDE$logLikelihood <- effs
  res_tableDE <- as.data.frame(lapply(res_tableDE, unlist))
  res_tableDE <- res_tableDE[order(res_tableDE$logLikelihood, decreasing = FALSE),]
  ...
  ggplot(res_tableDE,aes(x = log2FoldChange, y = -log10(padj))) +
    ...
    geom_point(aes(colour = logLikelihood),alpha = 0.5, size = 1.2) +
    #scale_color_viridis(begin = 0, end = 1, direction = -1, option = 'B', name = 'TIS Efficiency') +
    
    #scale_fill_viridis(begin = 0, end = 1, direction = -1, option = 'B', name = 'logLikelihood', na.value = NA) + 
    #scale_fill_gradient(low = "yellow", high = "red", na.value = NA)
