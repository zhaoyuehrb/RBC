...
plot_volcano <- function(res_tableDE, type, day,list_of_genes,label,folder_label,xlims,ylims){
  ...
  right <- unname(quantile(res_tableDE$log2FoldChange,0.9998))
  left <- unname(quantile(res_tableDE$log2FoldChange,0.0002))
  up <- unname(quantile(-log10(res_tableDE$padj),0.9998))
  ...
  ggplot(res_tableDE,aes(x = log2FoldChange, y = -log10(padj))) +
    ...
    geom_text_repel(#data = subset(res_tableDE, ((log2FoldChange > 1 | log2FoldChange < -1) & (FALSE))),
      ...
      data = subset(res_tableDE,  (SYMBOL %in% c('AHSP','VEGFA','ACTB','ALAS2','SLC4A1','TMEM175','TSPO2') | (log2FoldChange>right) | (log2FoldChange<left) | (-log10(padj)>up))),
...
plot_volcano(res_0_L24,'ER L24 T0',1,c(),'','',xlims=c(-10,10), ylims=c(0,10))

#plot_volcano(res_0_L24,'L24_0_label',1,c(subset(res_0_L24,log2FoldChange>1)),'','',xlims=c(-5,5), ylims=c(0,10))

plot_volcano(res_1_L24,'ER L24 T1',1,c(),'','',xlims=c(-10,10), ylims=c(0,5))
plot_volcano(res_2_L24,'ER L24 T2',1,c(),'','',xlims=c(-5,5), ylims=c(0,10))

plot_volcano(res_0_S15,'ER S15 T0',1,c(),'','',xlims=c(-13,13), ylims=c(0,30))
plot_volcano(res_1_S15,'ER S15 T1',1,c(),'','',xlims=c(-8,8), ylims=c(0,40))
plot_volcano(res_2_S15,'ER S15 T2',1,c(),'','',xlims=c(-8,8), ylims=c(0,20))

plot_volcano(res_0_S24,'ER S24 T0',1,c(),'','',xlims=c(-12,12), ylims=c(0,10))
plot_volcano(res_1_S24,'ER S24 T1',1,c(),'','',xlims=c(-7,7), ylims=c(0,10))
plot_volcano(res_2_S24,'ER S24 T2',1,c(),'','',xlims=c(-7,7), ylims=c(0,10))


plot_volcano(L24_0_S15,'L24 against S15 T0',1,c(),'','',xlims=c(-20,20),ylims=c(0,25))
plot_volcano(L24_1_S15,'L24 against S15 T1',1,c(),'','',xlims=c(-5,5),ylims=c(0,35))
plot_volcano(L24_2_S15,'L24 against S15 T2',1,c(),'','',xlims=c(-7,7),ylims=c(0,20))


plot_volcano(res_1_against_0_RNA,'RNA T1 against T0',1,c(),'','',xlims=c(-25,25), ylims=c(0,100))
plot_volcano(res_2_against_0_RNA,'RNA T2 against T0',1,c(),'','',xlims=c(-30,30), ylims=c(0,50))


plot_volcano(res_1_against_0_RPF,'RPF T1 against T0',1,c(),'','',xlims=c(-12,12), ylims=c(0,50))
plot_volcano(res_2_against_0_RPF,'RPF T2 against T0',1,c(),'','',xlims=c(-15,15), ylims=c(0,30))


plot_volcano(res_1_te_0,'TE T1 against T0',1,c(),'','',xlims=c(-23,23), ylims=c(0,20))
plot_volcano(res_2_te_0,'TE T2 against T0',1,c(),'','',xlims=c(-23,23), ylims=c(0,20))
