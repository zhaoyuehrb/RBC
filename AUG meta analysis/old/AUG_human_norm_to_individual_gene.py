...
acc2cdR = {}
totalMappedReads = 0
rpkmfile = sys.argv[4]#rootdirectory+'../geneTXCD_RPKMoutput_'+rootdirectory[:-1].split('/')[-1]+'.txt'
...
#############################################################################
##################################################################################
bgName = sys.argv[5] # 'background gene list.txt'
f = open(bgName,'r')
bgFile = f.readlines()
f.close()
        ...
        if ORFlength >= 400 and accNum in selectGeneAccDict:
            ...
            if accNum in exonHitsCollection:
                ...
                for exonHit in exonHits:
                    ...
                    if -102 <= shiftedHitStart < 303:
                        ...
                        baseToReads[shiftedHitStart] += exonHit.readNum() / acc2cdR[accNum]
                        ## now figure out codon
                        codon = shiftedHitStart/3
                        codonToReads[codon] += exonHit.readNum() / acc2cdR[accNum]
            ...
            if accNum in uniqRefseqHits:
                ...
                for refseqHit in refseqHits:
                    ...
                    if -102 <= shiftedHitStart < 303:
                        ## record, shifted position relative to cdStart
                        baseToReads[shiftedHitStart] += refseqHit.readNum() / acc2cdR[accNum]
                        ## now figure out codon
                        codon = shiftedHitStart/3
                        codonToReads[codon] += refseqHit.readNum() / acc2cdR[accNum]
                        if got == 0:
